...
### 分库
   如果当目前的数据库服务器无法满足一定并发的读和写时，就可以考虑分库。分库就是分出同一结构的备库存储不同的数据表信息，可以根据功能，热度，或者时间之类的划分不同的表到不同的库。这样可以有效减少增量数据写入时的锁对查询的影响；单表数量下降，查询对每个数据库实例所需扫描的记录数也下降。但是无法解决单表数据量太大的问题。
   
### 主从复制
   主从复制的原理就是，从机接受主机发送的binlog日志文件，binlog日志记录了主机对数据表的增删改事件，读取文件到中继日志中，然后加载中继日志中记录的增删改事件，进行同步到当前数据库。
   原则上，从只能对应一主，一主可以对应多从，每个服务server id 不一样，一般写操作只能给到主，读操作给到从机。
   主从之间最大问题就是延时，主更新数据后发送binlog到从有一定的时延，此时直接读从机的数据可能读到的是修改之前的数据，也就是过期读。解决这个问题有：同步复制，就是主库更新后发送binlog给从库，等从库同步完之后再返回客户端说成功，这个方案的问题缺陷很明显；半同步复制，主库将更新的binlog 发送给从库，从库接受到之后就返回成功为主库，之后从库再放进中继日志写入数据库，主库收到成功后也就返回给客户端说成功，较同步复制有一定的改良；多库并行复制之GTID，如果多个事务操作的表不同，则可以把这多个事务归到一个组，并给定一定的编号，发送到从库也同时执行多个并行的复制，因为他们之间的资源互不干扰所以不会有影响，如果有相同表的多个事务会被放到一个顺序的队列，保证顺序执行。
